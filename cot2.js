...
async function main() {
  ...
  const message = [
    ...
    {
      role: "user",
      content: "Write a code in JS to find sum of 1 to 5 ",
    },
  ];
  ...
  function extractJSON(text) {
    ...
    if (match) {
      try {
        ...
        console.warn("⚠️ JSON block found but parse failed");
      }
    }

    // If no JSON found but there's text, wrap it as OUTPUT
    if (text.trim()) {
      return { step: "OUTPUT", content: text.trim() };
    }
  ...
  while (true) {
    ...
    if (!parsedContent) {
      console.error("❌ Could not parse valid JSON — retrying...");
      continue; // Let it try again instead of exiting
    }

    // Store what the assistant said in conversation history
    message.push({
      role: "assistant",
    ...
    if (parsedContent.step === "START") {
      ...
      // Tell AI to move to THINK
      message.push({
        role: "developer",
        content: JSON.stringify({
          step: "THINK",
          content:
            "Now break down the problem into subproblems and think step-by-step.",
        }),
      });
      continue;
    }
    ...
    if (parsedContent.step === "EVALUATE") {
      ...
      // Tell AI to keep thinking (unless it’s ready for OUTPUT)
      message.push({
        role: "developer",
        content: JSON.stringify({
          step: "THINK",
          content: "Continue thinking further until the problem is solved.",
        }),
      });
      continue;
    }
